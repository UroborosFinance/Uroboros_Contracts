...
contract Treasury is Ownable {
    ...
    address public immutable Snake;
    ...
    address public SSnake;
    uint public sSnakeQueue; // Delays change to sSnake address
    ...
    /**
        @notice allow approved address to deposit an asset for SNAKE
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SNAKE needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Snake ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SNAKE for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        ISNAKEERC20( Snake ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( SSnake ).balanceOf( msg.sender ); // Can only borrow against sSnake held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SNAKE
        @param _amount uint
     */
    function repayDebtWithSNAKE( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISNAKEERC20( Snake ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Snake, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        require( isRewardManager[ msg.sender ], "Not approved" ); // Distributor & Bnb bond
        require( _amount <= excessReserves(), "Insufficient reserves" );

        IERC20Mintable( Snake ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Snake ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SNAKE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SNAKE decimals
            value_ = _amount.mul( 10 ** IERC20( Snake ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSNAKE ) { // 9
            sSnakeQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
